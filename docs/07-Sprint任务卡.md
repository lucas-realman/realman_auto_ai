...
> **机器约定**: W0=orchestrator(Orchestrator调度+Review+测试), W1=4090(aider), W2=mac_min_8T(aider), W3=gateway(aider), W4=data_center(aider), W5=aliyun-agent(预发布/Staging)
...
> **前置条件**: 6 台机器 SSH 互通、aider 已安装、Orchestrator 已部署
...
| **W4** | Git 裸仓库 + 6 机 clone | "在 mac_min_8T 上初始化 Git 裸仓库，编写 6 台机器的 clone 脚本" | `scripts/init_git.sh` | 6 台机器均可 `git push/pull` |
...
| **W4** | 开发自动化脚本 | "编写 6 台机器的一键启动/停止/状态检查脚本" | `scripts/dev_start.sh`, `scripts/dev_stop.sh`, `scripts/dev_status.sh` | 脚本可执行，状态检查输出所有服务状态 |
...
> **Day 4 收尾**: contracts/ 目录完整，6 台机器 `git pull` 后都能看到统一的接口定义。
...
| Day 3 | 6 台机器 Git 互通 | 检查 SSH 密钥、网络路由 |
...
| **W4** | Prometheus 部署 | "用 Docker 部署 Prometheus，配置 scrape 规则采集 6 台机器指标" | `deploy/monitoring/prometheus.yml`, `deploy/monitoring/docker-compose.yml` | Prometheus UI 可见各服务指标 |
